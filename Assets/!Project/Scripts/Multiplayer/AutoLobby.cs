...
public class AutoLobby : EOSLobby {
    ...
    private void OnJoinLobbySuccess(List<Attribute> attributes) {
        ...
        if(ConnectedLobbyDetails.GetMemberCount(new LobbyDetailsGetMemberCountOptions { }) > 1)
        {
        ...
        else // We take over hosting if we end up alone in a lobby
        {
            GetComponent<NetworkManager>().StartHost();
        }
    }
